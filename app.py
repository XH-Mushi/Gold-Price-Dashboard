...
with tab2:
    ...
    # 黄金价格趋势图
    st.subheader("黄金价格走势")
    fig_gold = px.line(gold_df, x='日期', y=['Open', 'Close', 'High', 'Low'],
                       title='黄金价格历史走势',
    ...
    with col_left:
        st.subheader("每日成交量")
        fig_volume = px.bar(gold_df, x='日期', y='Volume',
                            title='黄金交易成交量')
    ...
    with col_right:
        ...
        fig_volatility = px.line(gold_df, x='日期', y='波动幅度',
                                 title='日内价格波动幅度 (%)')
        st.plotly_chart(fig_volatility, use_container_width=True)

    # 显示原始数据
    st.subheader("黄金价格原始数据")
    st.dataframe(gold_df.style.highlight_max(axis=0), use_container_width=True)
